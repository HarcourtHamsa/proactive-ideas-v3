...
function CertificationSection({ certContent }: any) {
    const [isOpen, setIsOpen] = useState(false)
    const [certData, setCertData] = useState({
        header: certContent[0]?.header,
        subHeader: certContent[0]?.subHeader,
        isLoading: false
    })
    ...
    const saveChanges = async () => {
        ...
        client
            .patch(`${certContent[0]?._id}`) // Document ID to patch
            .set({
