...
function EditSingleCourse({ course }: any) {
  ...
  const [deleteSubSectionModalIsOpen, setDeleteSubSectionModalIsOpen] =
    useState(false);
  ...
  const { data: categories, isLoading: isFetchingCategories } =
    useFetchCategoriesQuery({ group: "course" });
  ...
  const { "0": deleteSectionFromDB, "1": deleteMutationStatus } =
    useDeleteSectionMutation();
  const { "0": deleteSubSectionFromDB, "1": deleteSubSectionMutationStatus } =
    useDeleteSubSectionMutation();
  ...
  const [fileSelectedForHeaderImage, setFileSelectedForHeaderImage] =
    useState<any>([]);
  ...
  const [editObjectiveModalIsOpen, setEditObjectiveModalIsOpen] =
    useState(false);
  ...
  const imageUpload = async (img: any) => {
    ...
    await axios
      .post("https://api.cloudinary.com/v1_1/dgn6edv1k/image/upload", formData)
      .then((response: any) => {
  ...
  const handleDrop = () => {
    ...
    const indexOfDraggedItem = shallowCopyOfSubSections.indexOf(
      dragItemRef.current
    );
    const indexOfDraggedOverItem = shallowCopyOfSubSections.indexOf(
      dragItemOverRef.current
    );

    // Remove the dragged item from the array
    const foundDraggedItem = shallowCopyOfSubSections.splice(
      indexOfDraggedItem,
      1
    )[0];

    // Insert the dragged item at the new position
    shallowCopyOfSubSections.splice(
    ...
    const indexOfActiveSection = courseInfoData.findIndex(
      (course) => course.title === shallowCopyOfActiveSections.title
    );

    // Create a shallow copy of courseInfoData and update the activeSection
    const shallowCopyOfCourseInfoData = [...courseInfoData];
    shallowCopyOfCourseInfoData[indexOfActiveSection] =
      shallowCopyOfActiveSections;
  ...
  function arrayToHTMLList(array: any[]) {
    const listItems = array
      .map((item: string) => `<li key={${Math.random()}}>${item}</li>`)
      .join("");
  ...
  const handleCourseUpdate = async () => {
    ...
    const courseShallowCopy = Object.assign(
      {},
      course,
      { ...generalInfoData },
      { sections: courseInfoData }
  ...
  const deleteSection = () => {
    deleteSectionFromDB({
      ...
      .finally(() => {
        ...
        const filteredSection = courseInfoData.filter(
          (section: any) => section.id !== currentSection.id
        );
  ...
  const handleDeleteObjective = (selectedObjective: string) => {
    ...
    const filteredObjectives = generalInfoData.objectives.filter(
      (objective: any) => objective !== selectedObjective
    );
  ...
  const deleteSubSection = async () => {
    ...
    const filteredSection = currentSection.sub_sections.filter(
      (ss: any) => ss._id !== currentSubSection._id
    );

    currentSection.sub_sections = filteredSection;

    setCourseInfoData((prevState: any) => {
      const courseIndex = prevState.findIndex(
        (el: any) => el.id === currentSection._id
      );
      ...
      courseInfoData.map((obj: any) => {
        ...
        if (shallowCopy._id === shallowCopyOfCurrentSection._id) {
          ...
          newObj = {
            ...shallowCopy,
            sub_sections: [
              ...shallowCopy.sub_sections,
              { ...subSectionData, _id: new ObjectID().toHexString() },
            ],
          };
  ...
  const addNewSection = async () => {
    agg = [...courseInfoData, newSectionData];
            ...
            <div className="px-4 my-8 space-y-3">
              {removeHeaderImage ? (
                <div>
                  ...
                  {isUploadingImage && (
                    <p className="mt-2">status: uploading image</p>
                  )}

                  {fileSelectedForHeaderImage?.map((f: any) => (
                    <div
                      key={Math.random()}
                      className="flex justify-between py-3 items-center px-2"
                    >
                      <p className="truncate overflow-hidden text-sm border bg-gray-100 p-1 w-fit rounded-md flex gap-8 items-center">
                        {f.name}
                ...
                <div
                  className="flex lg:h-[350px] h-[200px] flex-col bg-white rounded overflow-hidden mb-10"
                  onClick={() => setRemoveHeaderImage(true)}
                >
                  <figure className="relative h-0 pb-[56.25%] overflow-hidden">
                    <Image
                      className="absolute inset-0 w-full h-full object-cover bg-bottom transform transition duration-700 ease-out"
                      src={generalInfoData.header_image}
                      width="700"
                      height="1000"
                      alt="Course"
                      priority={true}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <CustomInput
                  ...
                  type="text"
                  name="author"
                  value={generalInfoData.author}
                  onChange={handleChange}
                />
                {/* <CustomInput label='Price' type='text' name='price' value={generalInfoData.price} onChange={handleChange} /> */}
                <div>
                  <label htmlFor="countries" className="block text-black">
                    Category
                  </label>
                  <select
                    id="countries"
                    className="bg-white border-2 text-gray-900 rounded focus:ring-blue-500 outline-blue-500 focus:border-blue-500 block w-full p-2.5"
                    name="category"
                    value={generalInfoData.category}
                    onChange={handleChange}
                  >
              ...
              <div className="">
                ...
                <textarea
                  id="message"
                  rows={4}
                  className="block p-2.5 w-full text-gray-900 bg-white rounded outline-blue-500 focus:ring-blue-500 focus:border-blue-500 border-2"
              ...
              <div className="">
                ...
                <ul className="mt-3">
                  {generalInfoData?.objectives?.map(
                    (objective: any, index: number) => {
                      return (
                        <li
                          key={Math.random()}
                          className="py-2 border px-4 my-2 rounded flex justify-between"
                        >
                          {objective}
                          ...
                          <span className="flex gap-4">
                            <TbPencil
                              ...
                              onClick={() => {
                                ...
                                setEditObjectiveModalIsOpen(
                                  !editObjectiveModalIsOpen
                                );
                              }}
                            />
                            <TbTrash
                ...
                <div key={Math.random()}>
                  <div className="flex items-center gap-1">
                    <div className="flex items-center w-full relative border rounded bg-white justify-between px-3">
                      ...
                      <span className="text-sm px-2 py border rounded-full bg-gray-50 -top-2 absolute">
                        Section
                      </span>
                    </div>
                      ...
                      >
                        ...
                        <div className="flex items-center mt-4 w-full ">
                          <div className="w-[100%] px-4 py-3 flex justify-between relative ml-auto rounded bg-gray-100 my-2 border">
                            ...
                            <span className="text-sm px-2 py border rounded-full bg-white -top-2 absolute">
                              Sub-Section
                            </span>
                          </div>
                          <TbPencil
              ...
              <Modal>
                <div>
                  ...
                  <textarea
                    ...
                    className="block p-2.5 w-full text-gray-900 bg-white rounded outline-blue-500 focus:ring-blue-500 focus:border-blue-500 border-2"
                    placeholder="Write your thoughts here..."
                    value={activeObjective.objective}
                    onChange={(e) => {
              ...
              <Modal>
                <div>
                  ...
                  <textarea
                    ...
                    className="block p-2.5 w-full text-gray-900 bg-white rounded outline-blue-500 focus:ring-blue-500 focus:border-blue-500 border-2"
                    placeholder="Write your thoughts here..."
                    value={activeSection.section.title}
                    onChange={(e) => {
              ...
              <Modal>
                ...
                <div className="text-center">
                  ...
                  <div className="mt-4 flex w-fit mx-auto">
                    <button
                      className="border px-3 mr-4 flex items-center gap-1 py-2 rounded"
                      onClick={deleteSection}
                    >
                      {deleteMutationStatus.isLoading && <Spinner />} Delete
                    </button>
                    <button
              ...
              <Modal>
                ...
                <div className="text-center">
                  ...
                  <div className="mt-4 flex mx-auto text-center gap-2">
                    ...
                    >
                      {deleteSubSectionMutationStatus.isLoading && <Spinner />}{" "}
                      Delete
                    </button>
                    <button
                      className="border w-[100%] px-3 py-2 rounded"
                      onClick={() => setDeleteSubSectionModalIsOpen(false)}
                    >
                      Cancel
                    </button>
                  </div>
              ...
              <div className="h-screen w-screen bg-transparent fixed top-0 z-[100] flex items-center justify-center transition duration-75">
                <div className="w-[100%] h-fit sm:w-screen xl:-translate-x-4 sm:h-screen overflow-y-scroll z-[100] bg-[#fff] shadow-md p-4 transition duration-75">
                  ...
                  <div className="mt-6 xl:w-[60%] w-[100%] mx-auto">
                    <ReactQuillEditor
                      onChange={(value: any) => {
                        const [title, content] = seperateBlogDataIntoComponents(
                          value as string
                        );
              ...
              <div className="h-screen w-screen bg-transparent fixed top-0 z-[100] transition duration-75">
                <div className="w-[100%] h-fit sm:w-screen xl:-translate-x-4 sm:h-screen overflow-y-scroll z-[100] bg-[#fff] shadow-md p-4 transition duration-75">
                  <div className=" w-fit ml-auto space-x-4 flex px-4">
                    ...
                    <button
                      className="py-1 px-4 border rounded flex items-center gap-2"
