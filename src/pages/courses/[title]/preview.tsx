...
import { useFetchCourseEnrollmentQuery, useFetchCoursePaymentListQuery, useFetchSingleCourseQuery } from "@/features/apiSlice";
                                                    ...
                                                    <Disclosure.Panel className="">
                                                        <div>
                                                            {course?.data?.sections.map((item: any, index: number) => (
                                                                <div
                                                                    key={Math.random()}
                                                                    className="px-4 py-2 hover:bg-zinc-100/10 rounded duration-200 cursor-pointer border-b-2"
                                                                >

                                                                    <p className="text-lg flex items-center gap-1 text-black">
                                                                        <span className="w-8 h-8 border flex justify-center items-center rounded-full">
                                                                            {index + 1}
                                                                        </span>
                            ...
                            <div className="flex flex-col h-full bg-[#fff] border rounded-md overflow-hidden">
                                <div className="flex-grow flex flex-col px-4 py-4">
                                    ...
                                    <div className="flex-grow mt-4">
                                        {
                                            !(parsedSubsriber) && <button className="py-3 mb-2 border-[#F08354] border text-[#F08354] uppercase text-sm tracking-wider hover:bg-orange-50 w-full rounded-md" onClick={() => {
                                                cookie?.user ? router.push({ pathname: `/courses/${course.data.title}/${course.data?.id}` }) : router.push(`/auth/login/?next=${router.asPath}`)
                                            }}>
                                                Preview Course - it&apos;s free
                                            </button>
                                        }
                                            ...
                                            cookie?.user ?
                                                parsedSubsriber ?
                                                    <button
                                                        ...
                                                        onClick={() => router.push({ pathname: `/courses/${course.data.title}/${course.data?.id}` })}
                                                    >
                                                        Continue learning
                                                    </button> :
                                                    <div className="">
